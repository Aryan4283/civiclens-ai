...
## 🚀 Live Demo
- **Frontend (Citizen App):** [Insert Firebase .web.app Link]
- **Backend (AI Engine):** [Insert Google Cloud Run .run.app Link]
- **Google Doc Report:** [Insert Google Doc Link]
...
Citizens can upload a photo of a pothole, leak, or hazard. Our **Observer Agent** (powered by Gemini) uses True Function Calling to extract structured data: categorizing the issue, scoring its severity (1-5), and generating a detailed description.

![AI Analysis](./screenshots/07-report-analysis.png)

### 2. AI-Enforced SLAs (Service Level Agreements)
Instead of just classifying an issue, our **Router Agent** cross-references a municipal rulebook to automatically assign a strict deadline (e.g., 24 hours for a severe water leak) and route it to the correct government department.
...
- **Google Gen AI SDK (Gemini):** Powered by `gemini-3.5-flash` for high-speed multi-modal vision, true function calling, and multilingual text processing. Includes a custom fallback mesh to `gemini-3.1-flash-lite` for rate-limit resilience.
